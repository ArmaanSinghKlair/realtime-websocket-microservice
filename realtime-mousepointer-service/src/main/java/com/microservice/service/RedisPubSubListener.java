 ...
 * 
 * Same bean for ALL listeners. Thread-safe due to lack of state in this class.
 */
@Component
			...
			synchronized(topicLock) {
				if(!WebSocketMessageHandler.topicMap.containsKey(topicId)) {
					...
					synchronized(redisTopicLock) {						
						redisContainer.removeMessageListener(redisPubSubListener, RedisConfig.redisPubSubTopicMap.remove(topicId));
					}
					return;
