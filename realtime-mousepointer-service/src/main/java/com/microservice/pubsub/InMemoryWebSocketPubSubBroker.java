 ...
 */
@Component
public class InMemoryWebSocketPubSubBroker {
//	private final Logger logger = LoggerFactory.getLogger(this.getClass());
	...
	public void publish(WebSocketMessage msg) {
		long publishTopicId = msg.getPublishTopicId();
		...
		synchronized(topicLock) {
			...
			synchronized(topic.getSubscriberSet()) {
				if(topic.getSubscriberSet().isEmpty()) {
					WebSocketMessageHandler.topicMap.remove(publishTopicId);
					return;
				}
			}
		
			WebSocketMessageHandler.topicMap.get(msg.getPublishTopicId()).publish(msg);
		}
	}
	 ...
	 * @param subscriberId
	 */
	public void subscribeToTopic(Long topicId, Long subscriberId, ApplicationContext appContext) {
		...
		synchronized(topicLock) {
			...
			WebSocketMessageHandler.topicMap.get(topicId).subscribeToTopic(subscriberId);
		}
	}
	...
	public void unsubscribeFromTopic(Long topicId, Long subscriberId) {
		...
		synchronized(topicLock) {
			...
			WebSocketMessageHandler.topicMap.get(topicId).unsubscribeFromTopic(subscriberId);
		}
	}
